...
def test_bankaccount_import_do_fileupload(client):
    ...
    assert BankAccount.objects.get(name="GLS Bank").entries.first().start == parse("2023-10-01").date()
    assert BankAccount.objects.get(name="GLS Bank").entries.first().end == parse("2023-11-01").date()
    ...
    assert (
        BankAccount.objects.get(name="GLS Investitionen/Rücklagen").entries.first().start == parse("2023-10-01").date()
    )
    assert BankAccount.objects.get(name="GLS Investitionen/Rücklagen").entries.first().end == parse("2023-11-01").date()
    ...
    assert BankAccount.objects.get(name="GLS Geschäftsguthaben").entries.first().start == parse("2023-10-01").date()
    assert BankAccount.objects.get(name="GLS Geschäftsguthaben").entries.first().end == parse("2023-11-01").date()
    ...
    assert BankAccount.objects.get(name="Bank für Sozialwirtschaft").entries.first().start == parse("2023-10-01").date()
    assert BankAccount.objects.get(name="Bank für Sozialwirtschaft").entries.first().end == parse("2023-11-01").date()
    ...
    assert (
        BankAccount.objects.get(name="Bank für Sozialwirt. Tagesgeld").entries.first().start
        == parse("2023-10-01").date()
    )
    assert (
        BankAccount.objects.get(name="Bank für Sozialwirt. Tagesgeld").entries.first().end == parse("2023-11-01").date()
    )
...
def test_bankaccount_charts_sum_balance_by_month_with_data(admin_client):
    ...
    BankAccountEntry.objects.create(bankaccount=ba1, start="2017-08-01", end="2017-10-22", balance="200")
    BankAccountEntry.objects.create(bankaccount=ba1, start="2018-01-01", end="2018-04-01", balance="100")
    BankAccountEntry.objects.create(bankaccount=ba1, start="2019-03-01", end="2020-04-01", balance="300")

    # with a second BankAccount
    ba2 = BankAccount.objects.create(name="ba2")
    BankAccountEntry.objects.create(bankaccount=ba2, start="2018-03-01", end="2018-05-01", balance="30")
