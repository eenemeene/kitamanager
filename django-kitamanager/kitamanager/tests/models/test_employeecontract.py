...
from kitamanager.models import Area, Employee, EmployeeContract
...
def test_employeecontract_create():
    ...
    _employeecontract_create(e1, start="2020-01-01", end="2020-12-31")
    _employeecontract_create(e1, start="2019-01-01", end="2019-12-31")
    _employeecontract_create(e1, start="2021-01-01", end="2021-12-31")

    # same employeecontract for other employee should work
    _employeecontract_create(e2, start="2020-01-01", end="2020-12-31")
...
def test_employeecontract_hours_total():
    ...
    ec1 = _employeecontract_create(
        e1, start="2020-01-01", end="2020-12-31", hours_child=1, hours_management=2, hours_team=2, hours_misc=2
    )
...
@pytest.mark.parametrize(
    ...
    [
        ...
        (
            ...
            "2015-01-01",
            {"e2": {"start": parse("2015-01-01").date(), "end": parse("2015-12-31").date()}},
        ),
        (
            {
                "e1": [["2021-01-01", "2021-12-31"], ["2022-01-01", "2022-12-31"]],
            ...
            "2022-01-01",
            {
                "e1": {"start": parse("2022-01-01").date(), "end": parse("2022-12-31").date()},
            },
        ),
    ],
)
...
def test_employeecontract_manager_by_date(setup, check_date, expected):
    ...
    for employee, contracts in setup.items():
        ...
        for contract_dates in contracts:
            _employeecontract_create(emp, start=contract_dates[0], end=contract_dates[1])
    ...
    for expected_employee, expected_contract in expected.items():
        ...
        assert (
            res.get(person__first_name=expected_employee, person__last_name=expected_employee).start
            == expected_contract["start"]
        )
        assert (
            res.get(person__first_name=expected_employee, person__last_name=expected_employee).end
            == expected_contract["end"]
        )
...
@pytest.mark.django_db
def test_employeecontract_get_latest_by():
    """
    Check the get_latest_by aka latest() queryset function
    """
    e = Employee.objects.create(first_name="1", last_name="11", birth_date="2017-10-22")

    # without any contract
    with pytest.raises(EmployeeContract.DoesNotExist):
        e.contracts.latest()

    # with single contract
    _employeecontract_create(e, **{"start": "2010-01-01", "end": "2020-01-01"})
    assert e.contracts.latest().start == parse("2010-01-01").date()

    _employeecontract_create(e, **{"start": "2020-01-01", "end": "2030-01-01"})
    assert e.contracts.latest().start == parse("2020-01-01").date()
