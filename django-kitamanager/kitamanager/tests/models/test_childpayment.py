...
def test_childpaymenttable_create():
    ...
    ChildPaymentTable.objects.create(plan=p1, date=["2020-01-01", "2020-12-31"])
    ChildPaymentTable.objects.create(plan=p1, date=["2021-01-01", "2021-12-31"])

    # same tables, but for the other plan should work
    ChildPaymentTable.objects.create(plan=p2, date=["2020-01-01", "2020-12-31"])
    ChildPaymentTable.objects.create(plan=p2, date=["2021-01-01", "2021-12-31"])

    # overlapping date should not work
    with pytest.raises(IntegrityError):
        ChildPaymentTable.objects.create(plan=p2, date=["2021-06-01", "2023-12-31"])
...
def test_childpaymenttableentry_create():
    ...
    t11 = ChildPaymentTable.objects.create(plan=p1, date=["2020-01-01", "2020-12-31"])
    t12 = ChildPaymentTable.objects.create(plan=p1, date=["2021-01-01", "2021-12-31"])

    # same tables, but for the other plan should work
    t21 = ChildPaymentTable.objects.create(plan=p2, date=["2020-01-01", "2020-12-31"])
    t22 = ChildPaymentTable.objects.create(plan=p2, date=["2021-01-01", "2021-12-31"])
